...
def test_get_mask_card_number(number_card: str) -> None:
    ...
    # assert get_mask_card_number(number_card) == "Номер карты должен быть 16 цифр"


def test_get_mask_account(number_account: str) -> None:
    assert get_mask_account(number_account) == "**4305"
    # assert get_mask_account(number_account) == "Номер карты должен быть не меннее 4 цифр"
